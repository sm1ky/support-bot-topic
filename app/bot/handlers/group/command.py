...
async def close_all_topics(message: Message, manager: Manager, redis: RedisStorage) -> None:
    """
    ...
    if message.from_user.id != manager.config.bot.DEV_ID:
        logging.info(f"Отказано в доступе: {message.from_user.id} != {manager.config.bot.DEV_ID}")
    ...
    for user_id in user_ids:
        try:
            ...
            if user_data.topic_status not in ("new", "open"):
                already_closed += 1
                logging.info(f"Топик пользователя {user_id} уже закрыт: {user_data.topic_status}")
                continue
            ...
            logging.info(f"Изменен статус пользователя {user_id} с {old_status} на closed")
            ...
            try:
                ...
                logging.error(f"Ошибка при изменении названия топика для {user_id}: {e}")
                errors += 1
                continue
            ...
            logging.error(f"Непредвиденная ошибка при обработке пользователя {user_id}: {e}")
            errors += 1
    ...
    logging.info(f"Завершено закрытие топиков: {closed} закрыто, {already_closed} уже были закрыты, {errors} ошибок")
    await message.delete()
